...
class GridWorldFeatureExtractor13(FeatureExtractor):
    ...
    def extract(self, state, action):
        ...
        for a in self.mdp.get_actions():
            if a == action and state != GridWorld.TERMINAL:
                ...
                ## Linear feature based on the difference between x-coordinate and goal x-coordinate
                feature_values += [state[x] - goal[x]]

                # Linear feature based on the difference between y-coordinate and goal y-coordinate
                feature_values += [state[y] - goal[y]]
            else:
