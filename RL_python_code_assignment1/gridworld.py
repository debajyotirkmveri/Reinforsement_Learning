...
class GridWorld(MDP):
    ...
    def is_blocked(self, state, action):
        # Implement the logic to check if an action is blocked.
        # For example, you can use the 'self.blocked_states' attribute to check if a state is blocked.
        x, y = state
        if action == self.UP and (x, y + 1) in self.blocked_states:
            return True
        elif action == self.DOWN and (x, y - 1) in self.blocked_states:
            return True
        elif action == self.LEFT and (x - 1, y) in self.blocked_states:
            return True
        elif action == self.RIGHT and (x + 1, y) in self.blocked_states:
            return True
        return False
...
if __name__ == "__main__":
    small = GridWorld(width= 6, height=6)
    small.visualise_as_image(title="Small")

    medium = gridworld = GridWorld(width = 6, height= 6)
